...
function MemberList(props) {
    ...
    const curfilteredCustomers = filteredCustomers.slice(
        indexOfFirstItem,
        indexOfLastItem
    );
    ...
    const handleMoveToDeleted = async () => {
        ...
        try {
            ...
            const res = await axios.post(
                `${bkURL}/admin/member/moveToDeleted`,
            ...
            setArr((prev) =>
                prev.map((member) =>
                    selectedCustomers.includes(member.customer_id)
                        ? { ...member }
                        : member
                )
            );
    ...
    const handleSearch = () => {
        ...
        if (searchType === "customer_name") {
            member = member.filter((member) =>
                member.customer_name.includes(searchValue)
            );
        } else if (searchType === "contact_number") {
            member = member.filter((member) =>
                member.contact_number.includes(searchValue)
            );
        }
    ...
    const handleSelectAll = () => {
        ...
        if (!selectAll) {
            ...
            setSelectedCustomers(
                filteredCustomers.map((member) => member.customer_id)
            );
        } else {
    ...
    const handleSelectEach = (customer_id) => {
        if (selectedCustomers.includes(customer_id)) {
            // 이미 체크가 된 고객이라면
            setSelectedCustomers(
                selectedCustomers.filter((id) => id !== customer_id)
            );
            // 체크박스 재클릭 시 체크 해제
        } else {
                    ...
                    <tr key={mm.customer_id}>
                        <td>
                            <input
                                ...
                                onChange={() =>
                                    handleSelectEach(mm.customer_id)
                                } // 체크 상태에 따라 handleSelectEach 작동
                            />
                        </td>
                        <td>
                            <Link
