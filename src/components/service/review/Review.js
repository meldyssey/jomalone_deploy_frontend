...
function fileGo(file) {
    ...
    return (
        <img src={`${bkURL}/imgs/product/default.jpg`} alt="Default Product" />
    );
}
...
const Review = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!contentRegex.test(content)) {
            setContentError(
                "상품 후기는 10자 이상 500자 이하로 입력해 주세요."
            );
            return;
        }
        setContentError("");
