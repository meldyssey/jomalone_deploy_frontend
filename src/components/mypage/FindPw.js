...
function FindPw() {
    ...
    const handleFindPassword = async () => {
        ...
        try {
            const res = await axios.post(`${bkURL}/findPw`, {
                email,
    ...
    const handleResetPassword = async () => {
        ...
        if (
            !/^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%~]).{12,16}$/.test(newPassword)
        ) {
            alert(
                "비밀번호는 영문 대소문자, 숫자, 특수문자(!,@,#,$,%,~)를 포함한 12~16자 이내여야 합니다."
            );
            return;
        }
        try {
            const res = await axios.post(`${bkURL}/resetPw`, {
                email,
                newPassword,
                newPassword2,
            });
